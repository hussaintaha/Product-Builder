export const action = async ({ request }) => {
    ...
    try {
        ...
        return new Response(JSON.stringify({ success: true, message: "Subscription has been resumed.", data: replitData }), { status: 200 })

    } catch (error) {
        if (error instanceof Error) {
            console.log(`Error while resuming subscription: ${error.message}`);
        } else {
