...
const App = () => {
  ...
  useEffect(() => {
    const selectors = ["#PRODUCT_BUILDER_SUBSCRIPTIONS", "#WIDGET"];
    let foundData = null;
  ...
  const fetchSubscriptionHandler = useCallback(
    async (attempt = 1) => {
      ...
      try {
        ...
        console.log(`Subscription Attempt ${attempt}:`, {
          success,
          error,
          message,
          subscription,
        ...
        if (!success && error) {
          if (attempt < 3) {
            setTimeout(() => fetchSubscriptionHandler(attempt + 1), 1500);
          } else {
            toast.error(error);
            setShow(false);
          ...
          if (
            ...
            subscription.actual_attempts !== subscription.used_attempt
          ) {
            setShow(true);
            toast.success(message || "Subscription found", {
              position: "top-center", progress: true
            });
          }
  ...
  const subscriptionRoot = document.querySelector(
